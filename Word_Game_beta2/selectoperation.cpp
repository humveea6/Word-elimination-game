    ...
    for(int i=0;i<20;i++){
        ...
        connect(newbutton,&QPushButton::clicked, [=](){
            ...
            emit this->sel_to_game(current_Player.name,current_Player.pwd,current_Player.cgs,
                                   current_Player.jyvalue,current_Player.level,i+1);
        });
    }
...
void Selectoperation::receive_Show(QString user, QString pwd, int cgs, int jyvalue, int level)      //信号槽
{
    ...
    QString s="欢迎您，玩家"; s+=QString(user);
