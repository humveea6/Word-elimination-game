...
public slots:
    ...
    void time_out1();
    void click_button1();
    void receive_from_last(QString user,QString pwd,int cgs,int jyvalue,int level,int hard);

signals:
    void play_return_sel(QString user,QString pwd,int cgs,int jyvalue,int level);
    void go_to_next(QString user,QString pwd,int cgs,int jyvalue,int level,int hard);
